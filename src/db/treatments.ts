...
export function createTreatment(data: Omit<Treatment, "id">): Promise<Treatment> {
  return withDb(async (db) => {
    const result = await db.run(
      "INSERT INTO treatments (label, frequency, reminder_time, reminder_enabled, created_at) VALUES (?, ?, ?, ?, ?)",
      [data.label, data.frequency, data.reminderTime, data.reminderEnabled ? 1 : 0, data.createdAt],
    );
...
export function updateTreatment(
  ...
  return withDbVoid(async (db) => {
    ...
    if (data.reminderEnabled !== undefined) { fields.push("reminder_enabled = ?"); values.push(String(data.reminderEnabled ? 1 : 0)); }
