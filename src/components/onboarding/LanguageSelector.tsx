...
export function LanguageSelector({ onSelect }: LanguageSelectorProps) {
  ...
  return (
    <Box sx={{ height: "100dvh", display: "flex", flexDirection: "column", px: 3, pt: 6, pb: "calc(32px + env(safe-area-inset-bottom))" }}>
      ...
      <Box sx={{ display: "grid", gridTemplateColumns: "1fr 1fr", gap: 1.5, flex: 1 }}>
        {LANGUAGES.map(({ locale, flag, name }) => {
          const selected = current === locale || (locale === "en" && !LANGUAGES.some(l => l.locale === current));
          return (
            <Box
              key={locale}
              role="button"
            ...
            >
              ...
              <Typography variant="body2" sx={{ fontWeight: selected ? 700 : 400 }}>{name}</Typography>
            </Box>
          );
        })}
      </Box>
