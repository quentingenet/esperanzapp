...
export function ReduceHabitsTab() {
  ...
  const { getPermissionStatus, requestPermission, getExactAlarmStatus, openExactAlarmSettings } =
    useNotifications();
  ...
  useEffect(() => {
    if (habitsError) toast.error(t("common.error"));
  }, [habitsError, t]);
  useEffect(() => {
    void getPermissionStatus().then((status) => {
      setNotifPermGranted(status);
    });
  }, [getPermissionStatus]);
        ...
        void getStatsBatch([habit.id])
          .then((freshStats) => {
            ...
            void cancelMilestoneNotifications(habit.id)
              ...
              .catch((e: unknown) => {
                logError("ReduceHabitsTab.handleRelapse.notifications", e);
              });
          })
          .catch((e: unknown) => {
            logError("ReduceHabitsTab.handleRelapse.getStats", e);
          });
        void loadHabits();
        setDetailHabit(null);
      })
      .catch((e: unknown) => {
        logError("ReduceHabitsTab.handleRelapse", e);
  ...
  const handleDeleteConfirmed = () => {
    ...
    void deleteHabit(deleteTarget.id)
      ...
      .catch((e: unknown) => {
        logError("ReduceHabitsTab.handleDelete", e);
        toast.error(t("common.error"));
      });
  ...
  const handleExitSort = useCallback(() => {
    ...
    void saveHabitsOrder()
      .catch((e: unknown) => {
        logError("ReduceHabitsTab.saveHabitsOrder", e);
        toast.error(t("common.error"));
        void loadHabits();
      })
      .finally(() => {
  ...
  return (
    ...
    >
      ...
      {habits.length === 0 && !habitsLoading && statsMap !== null && (
          ...
          renderItem={(h, handleProps) => {
            ...
            return (
              <HabitCard
                ...
                onClick={() => {
                  if (!sortMode) setDetailHabit({ habit: h, stats });
                }}
                onDelete={() => {
                  setDeleteTarget(h);
                }}
      ...
      <HabitForm
        ...
        onSubmit={(data) => {
          void addHabitWithInitialLog({ ...data, createdAt: new Date().toISOString() })
            ...
            .catch((e: unknown) => {
              logError("ReduceHabitsTab.addHabit", e);
              toast.error(t("common.error"));
            });
        }}
      />
      ...
      {detailHabit && (
        <HabitDetailModal
          ...
          onRelapse={(date) => {
            handleRelapse(detailHabit.habit, date);
          }}
        />
      )}
