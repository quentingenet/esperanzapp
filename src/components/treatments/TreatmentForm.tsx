      ...
      <Drawer anchor="bottom" open={open} onClose={() => { setOpen(false); }} slotProps={{ paper: { sx: { borderRadius: "16px 16px 0 0", maxHeight: "80dvh" } } }}>
        <LocalizationProvider dateAdapter={AdapterDateFns} adapterLocale={dateLocale}>
          <Box sx={{ px: 2, pt: 2, pb: "calc(24px + env(safe-area-inset-bottom))" }}>
            ...
            />
            <Select fullWidth value={frequency} onChange={(e) => { setFrequency(e.target.value); }} sx={{ mb: 2 }}>
              {FREQUENCIES.map((f) => (
            ...
            <Button
              ...
              disabled={!label.trim() || (reminderEnabled && !time)}
              aria-label={t("common.save")} sx={{ minHeight: 48, borderRadius: 2, mt: reminderEnabled ? 0 : 2 }}
            >
              {t("common.save")}
            </Button>
          </Box>
